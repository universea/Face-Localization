...
class MobileNetV3():
	def __init__(self, img, points_num, img_shape,model_mode="SMALL"):
		self.img = img
	...
	def large_net(self,name="large_net"):
		print("self.img.shape",self.img.shape)
		# 112*112
		conv1 = fluid.layers.conv2d(self.img,
										num_filters=16,
										filter_size=3,
										stride=2,
										padding=1,
...
def build_model(img,landmark,points_num, img_shape):
	
	model = MobileNetV3(img, points_num, img_shape)
	
	features ,landmarks_pre = model.large_net()
	
	
	#landmarks_loss = tf.reduce_sum(tf.square(landmarks_pre - landmark), axis=1)
	#landmarks_loss = tf.reduce_mean(landmarks_loss)
	
	loss_sub = fluid.layers.elementwise_sub(landmarks_pre, landmark)
	loss_square = fluid.layers.square(loss_sub)
	# loss
	landmarks_loss = fluid.layers.reduce_sum(loss_square, dim=1)
	landmarks_loss = fluid.layers.reduce_mean(landmarks_loss)	
